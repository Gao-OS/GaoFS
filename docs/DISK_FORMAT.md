...
| 196 | 4 | header_size | u32 | must be 196 in v1 |
...
| Field | Type | Notes |
| --- | --- | --- |
| rec_len | u32 | total bytes following this field |
| rec_type | u16 | enum |
| rec_flags | u16 | reserved |
| seq | u64 | record sequence |
| txn_id | u64 | transaction identifier |
| payload_crc32c | u32 | CRC32C of payload bytes |
| payload | bytes | `rec_len - header_bytes` |
...
Note: Dirent key includes both hash and bytes to avoid hash collision ambiguity.
